    ...
    def delta_attr(deltas = {})
        deltas.each do |attr, delta|
            if @attributes.has_key? attr
                ...
                print "The ", @name, "'s ", attr, " goes ", 
                if delta > 0 then "up" 
                else "down" end, " by ", delta, ".", $/
            end
        end
    end
...
# A Location that must be unlocked before going there
class LockedLocation < Location
    attr_reader :is_unlocked
    attr_reader :key
    def initialize(name, desc, key_name)
        super(name, desc)
        @key = key_name
        @is_unlocked = false
    end
    ...
    def try_unlock()
        ...
        Game.player.inventory.each do |item|
            if item.name == @key
                found = true
                @is_unlocked = true
                Game.player.inventory.delete_at(Game.player.inventory.index(item))
            end
        end
        return @is_unlocked
    end
...
class Help < Action
    ...
    def do(args = [])
        if (args.length == 0)
            Game.pretty_print(["Enter commands of the form <verb> <object>. For example, to walk between areas, use 'go north'.", "Available commands: "] + Game.actions.keys.map{|cmd| " - " + cmd}, "General help")
        else
            cmd = args[0].downcase
    ...
    def do(args = [])
        ...
        if Game.location.doors.has_key?(args[0])
            ...
            if next_location.is_a? LockedLocation
                if !next_location.is_unlocked
                    proceed = false
                    print args[0], " is locked. You need to unlock it with a ", next_location.key, " before you go ", args[0], ".", $/
                end
            end
            if proceed
...
class Game
    ...
    def main_loop()
        ...
        until false do
            if game_over
                puts "Congratulations, you've beat the game!"
                break
            end
            inp = Game.prompt
            args = inp.split(" ")
